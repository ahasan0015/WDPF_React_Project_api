...
function EditPaymentStatu() {
    const [paymentStatu, setPaymentStatu] = useState<PaymentStatu>(paymentStatuDefault);
    const { id } = useParams();

    useEffect(() => {
        document.title = "Edit PaymentStatu";
        getDataById();
        getPayment_statuss();
    }, []);

    const getDataById = () => {
        api.get("payment-status/" + id)
        .then((res) => {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        api.put("edit-payment-statu?id=" + id, paymentStatu)
        .then((res) => {
    ...
    return (
        ...
        <div className="container-xxl flex-grow-1 container-p-y">
          <h4 className="fw-bold py-3 mb-4">
            <Link to="/payment-status" className="text-muted fw-light">PaymentStatus /</Link> Edit
          </h4>
          <div className="card mt-3">
            <h5 className="card-header">Edit PaymentStatu</h5>
            <div className="card-body">
                <form onSubmit={handleSubmit}>
                    <div className="mb-3">
                        <label className="form-label">Payment Status Id</label>
                        <select name="payment_status_id" className="form-select"
                        value={paymentStatu.payment_status_id}
                        onChange={(e) => setPaymentStatu({...paymentStatu, payment_status_id: parseInt(e.target.value)})}>
                            {
...
export default EditPaymentStatu;