...
function EditPassenger() {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        api.put("edit-passenger?id=" + id, passenger)
        .then((res) => {
    ...
    return (
        ...
        <div className="container-xxl flex-grow-1 container-p-y">
          <h4 className="fw-bold py-3 mb-4">
            <Link to="/passengers" className="text-muted fw-light">Passengers /</Link> Edit
          </h4>
          <div className="card mt-3">
            <h5 className="card-header">Edit Passenger</h5>
            <div className="card-body">
                <form onSubmit={handleSubmit}>
                    <div className="mb-3">
                        <label className="form-label">Passenger Id</label>
                        <select name="passenger_id" className="form-select"
                        value={passenger.passenger_id}
                        onChange={(e) => setPassenger({...passenger, passenger_id: parseInt(e.target.value)})}>
                            {
                                passengers.map((item) =>
                    ...
                    <div className="mb-3">
                        ...
                        onChange={(e) => setPassenger({...passenger, booking_id: parseInt(e.target.value)})}>
                            {
                                bookings.map((item) =>
                                    <option value={item.id} key={item.id}>{item.name}</option>
                                )
