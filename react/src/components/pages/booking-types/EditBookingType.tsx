...
function EditBookingType() {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        api.put("edit-booking-type?id=" + id, bookingType)
        .then((res) => {
    ...
    return (
        ...
        <div className="container-xxl flex-grow-1 container-p-y">
          <h4 className="fw-bold py-3 mb-4">
            <Link to="/booking-types" className="text-muted fw-light">BookingTypes /</Link> Edit
          </h4>
          <div className="card mt-3">
            <h5 className="card-header">Edit BookingType</h5>
            <div className="card-body">
                <form onSubmit={handleSubmit}>
                    <div className="mb-3">
                        <label className="form-label">Booking Type Id</label>
                        <select name="booking_type_id" className="form-select"
                        value={bookingType.booking_type_id}
                        onChange={(e) => setBookingType({...bookingType, booking_type_id: parseInt(e.target.value)})}>
                            {
